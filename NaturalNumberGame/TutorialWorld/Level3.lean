...
numbers (we also need recursion, but we can ignore it for now). The first axiom says that 0 is a
...
if we have infinitely many true/false statements `P(0)`, `P(1)`, `P(2)`, and so on, and if
`P(0)` is true, and if for every natural number `d` we know that `P(d)` implies `P(succ(d))`, then
...
deduce that all the dominos will fall down. One can also think of it as saying that every natural
...
Let's practice our use of the `rw` tactic in the following example. Our hypothesis `h` is a proof that
`succ(a) = b` and we want to prove that `succ(succ(a))=succ(b)`. In words, we're going to prove that if
`b` is the number after `a` then `succ(b)` is the number after `succ(a)`. Now here's a tricky question. If
our goal is `⊢ succ (succ a) = succ b`, and our hypothesis is `h : succ a = b`, then what will the goal
change to when we type

`rw [h]`

Remember that `rw [h]` will look for the left hand side of `h` in the goal, and will replace it with
...
Important note : the tactic `rw` expects a proof afterwards (e.g. `rw [h1]`). But `rfl` is just `rfl`.

You may be wondering whether we could have just substituted in the definition of `b` and proved the
goal that way. To do that, we would want to replace the right hand side of `h` with the left hand
...
addition, and then we'll be ready to enter Addition World.
...
Now you are ready for [Level4.lean](./Level4.lean).
-/
